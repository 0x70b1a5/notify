...
export default function Settings() {
  const { notifications, settings, setSettings } = useNotifyStore()
  ...
  return <div className="flex-col-center gap-2 grow self-stretch">
    ...
    <div className="flex-col-center gap-2">
      {availableProcesses.map((process) => (
        <div key={process} className="flex-center gap-2 bg-white/10 p-2 rounded-md">
          <h2>{process}</h2>
            ...
            <div key={settingName} className="flex-center gap-2">
              ...
              {typeof settingValue === 'boolean' && <div className="flex-center">
                <button
                  ...
                  onClick={() => setSettings({ ...settings, [process]: { ...settings[process], [settingName]: !settingValue } })}
                >
              ...
              {typeof settingValue === 'number' && <input
                type="number"
                value={settingValue}
                onChange={(e) => setSettings({ ...settings, [process]: { ...settings[process], [settingName]: e.target.value } })}
              />}
